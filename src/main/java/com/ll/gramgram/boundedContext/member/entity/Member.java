...
@ToString // 디버그를 위한
@Entity // 아래 클래스는 member 테이블과 대응되고, 아래 클래스의 객체는 테이블의 row(튜플)와 대응된다.
@Getter // 아래 필드에 대해서 전부다 게터를 만든다. private Long id; => public Long getId() { ... }
public class Member {

    @Id     //Primary Key
    @GeneratedValue(strategy = IDENTITY)        //자동으로 1씩 증가
    private Long id;            //주키 타입은 Long -> JpaRepository 선언시 사용.

    @CreatedDate // 아래 칼럼에는 값이 자동으로 들어간다.(INSERT 할 때) -> 생성일자
    private LocalDateTime createDate;

    @LastModifiedDate // 아래 칼럼에는 값이 자동으로 들어간다.(UPDATE 할 때 마다) -> 수정일자
    private LocalDateTime modifyDate;

    private String providerTypeCode; // 일반회원인지, 카카오로 가입한 회원인지, 구글로 가입한 회원인지 -> 회원가입 종류

    @Column(unique = true)     //유일한(유니크한) 값만 들어올 수 있다. (중복값 X)
    ...
    private InstaMember instaMember;        //한 멤버당 인스타 아이디는 하나만 가능.
